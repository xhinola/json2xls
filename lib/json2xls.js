...
transform.prepareJson = function(json,config) {
    ...
    var fieldStyleIndexMap = conf.fieldStyleIndexMap || {};
    var captionStyleIndexMap = conf.captionStyleIndexMap || {};
    var hasStyleSheet = !!conf.style;
    var types = [];
    ...
    res.cols = fields.map(function(key,i) {
        return {
            caption: key,
            captionStyleIndex: (hasStyleSheet && captionStyleIndexMap.hasOwnProperty(key)) ? captionStyleIndexMap[key] : undefined,
            type: getType(jsonArr[0][key],types[i]),
            beforeCellWrite: function(row, cellData, eOpt){
                eOpt.cellType = getType(cellData,types[i]);
                if (hasStyleSheet && fieldStyleIndexMap.hasOwnProperty(key)) eOpt.styleIndex = fieldStyleIndexMap[key];
                return cellData;
            }
        };
    });
